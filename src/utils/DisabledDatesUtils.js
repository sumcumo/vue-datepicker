const checkDateSpecific = (date, disabledDates, utils) => {
  if (
    typeof disabledDates.dates !== 'undefined' &&
    disabledDates.dates.length
  ) {
...
const checkDateDisabledFromTo = (date, disabledDates) => {
  if (
    typeof disabledDates.to !== 'undefined' &&
    disabledDates.to &&
    date < disabledDates.to
  ) {
    return true
  }
  if (
...
const checkDateRange = (date, disabledDates) => {
  if (
    typeof disabledDates.ranges !== 'undefined' &&
    disabledDates.ranges.length
  ) {
